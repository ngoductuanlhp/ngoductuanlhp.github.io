...
- `_data/publications.yml`: list of papers, rendered in file order (newest first by convention). Fields:
  ...
  - `description`: one sentence, **hidden by default** behind a "TL;DR" pill button on its own line under the links row (JS in `js/index.js`; the collapse animation uses the `grid-template-rows: 0fr → 1fr` trick in `.tldr`). It supports markdown. Wrap 1–2 key phrases in `**…**` and they render in bold. `cv.html` strips the markup to plain text.
